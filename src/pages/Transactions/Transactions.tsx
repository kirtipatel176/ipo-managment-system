...
export const Transactions: React.FC = () => {
  ...
  const transactions = useLiveQuery(async () => {
    ...
    // Populate entity names for better display
    return Promise.all(txs.reverse().map(async tx => {
      let ipoName = '-';
      let fromName = '-';
      let toName = '-';
      ...
      if (tx.fromBankAccountId) {
        const bank = await db.bankAccounts.get(tx.fromBankAccountId);
        if (bank) fromName = bank.bankName;
      } else if (tx.fromPersonId) {
        const person = await db.people.get(tx.fromPersonId);
        if (person) fromName = person.fullName;
      }
      
      if (tx.toBankAccountId) {
        const bank = await db.bankAccounts.get(tx.toBankAccountId);
        if (bank) toName = bank.bankName;
      } else if (tx.toPersonId) {
        const person = await db.people.get(tx.toPersonId);
