...
import {
  ...
  mapAllocation, mapHolding, mapSale
} from '../../lib/mappers';
...
export function useDashboardMetrics(ipoId?: number | null) {
  return useQuery<DashboardMetrics | undefined>({
    queryKey: ['dashboardMetrics', ipoId],
    queryFn: async () => {
      const [
        { data: iposData },
        { data: peopleData },
      ...
      const bankBalanceRows: BankBalanceRow[] = allBanks.map(b => {
        const txsForBank = allTxs.filter(t => t.fromBankAccountId === b.id || t.toBankAccountId === b.id);
        ...
        const ipoBlockedFromBank = allAllocs
          .filter(a => a.purpose === 'IPO_BLOCKED' && a.currentHolderType === 'BANK' && a.currentHolderId === b.id)
      ...
      const ipoBlockedTotal = allAllocs.filter(a => a.purpose === 'IPO_BLOCKED').reduce((s, a) => s + a.amount, 0);
      const friendMoney = allAllocs.filter(a => a.purpose === 'UNALLOCATED' && a.currentHolderType === 'PERSON').reduce((s, a) => s + a.amount, 0);
      const invested = allAllocs.filter(a => a.purpose === 'INVESTED').reduce((s, a) => s + a.amount, 0);
      ...
      const friendsMoneyRows: FriendMoneyRow[] = allPeople.map(p => {
    ...
    // Optional: Refresh every few seconds or rely on invalidation
